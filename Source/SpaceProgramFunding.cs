...
namespace SpaceProgramFunding.Source
{
	...
	public class SpaceProgramFunding : MonoBehaviour
	{
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> Executes the settings applied action which occurs at campaign start or when the player changes
		/// 		  the settings within the game.</summary>
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> Called when the game scene is loaded. We want the mod's pop-up windows to close when a the scene
		/// 		  changes.</summary>
		///
		...
		public float GrossFunding()
		{
			...
			return Reputation.CurrentRep * Instance._settings.fundingRepMultiplier;
		}
		...
		{
			if (Instance == null) return 0;
			return Instance._settings.fundingIntervalDays * dayLength;
		}


		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> Fetch time data for the home-world. This is usually Kerbin, but can change with some planet-pack
		/// 		  mods. By fetching the data in this way the timing values for days remains true no matter if
		/// 		  the home-world has changed.</summary>
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> There is a quirk with the game such that modules get saved/loaded around the SPH or VAB but other
		...
		public int InactiveCostWages()
		{
			...
			if (Instance._misc.baseKerbalWage == 0) return 0;
			...
			foreach (var p in crew) {
				...
				if (p.rosterStatus == ProtoCrewMember.RosterStatus.Available) {
					kerbal_wage = kerbal_level * Instance._misc.baseKerbalWage;
				}
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> Kerbal astronauts that are on missions (not in Astronaut complex) are paid at a different rate.
		/// 		  Usually higher to account for "hazard pay".</summary>
		///
		...
		public int ActiveCostWages()
		{
			...
			if (Instance._misc.assignedKerbalWage == 0) return 0;
			...
			foreach (var p in crew) {
				...
				if (p.rosterStatus == ProtoCrewMember.RosterStatus.Assigned) {
					kerbal_wage = kerbal_level * Instance._misc.assignedKerbalWage;
				}
		...
		public int CostVessels()
		{
			...
			if (Instance._misc.activeVesselCost == 0) return 0;
			...
			return vessels.Sum(v => (int) (v.GetTotalMass() / 100.0 * Instance._misc.activeVesselCost));
		}
		...
		public int CostLaunches()
		{
			...
			var costs = 0;
			if (Instance._misc.launchCostsLaunchPad + Instance._misc.launchCostsRunway > 0) costs = launchCostsAccumulator;
		...
		public int CostBuildings()
		{
			...
			return !Instance._maintenance.isBuildingCostsEnabled ? 0 : _buildingCostsArchive;
		}
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> Performs the main funding action. This is where funds are collected from the Kerbal government
		/// 		  and distributed to all the necessary recipients. The core function of this mod is handled by
		/// 		  the logic in this method.</summary>
		private void FundingOperation()
		{
			 ...
			 * covering is enabled.
			 */
			if (Instance._settings.isCostsCovered) {
				if (net_funding < 0) {
					net_funding = 0;
				}
			}
			 ...
			 */
			if (net_funding < 0 && Instance._settings.isCostsCovered) net_funding = 0;
			 ...
			 */
			if (Instance._settings.repDecayRate > 0) {
				if (Reputation.CurrentRep > Instance._settings.minimumRep) {
					var amount_to_decay = Instance._settings.repDecayRate;
					if (amount_to_decay > 0) {
						Reputation.Instance.AddReputation(-amount_to_decay, TransactionReasons.Strategies);
						if (Reputation.CurrentRep < Instance._settings.minimumRep) {
							Reputation.Instance.SetReputation(Instance._settings.minimumRep, TransactionReasons.Strategies);
						}
					}
				}
			}
			 ...
			 */
			if (Instance._settings.isReputationAllowed) {
			 ...
			 */
			if (Instance._misc.isScienceAllowed) {
			 ...
			 */
			if (Instance._misc.bigProjectMultiple > 0) {
		...
		private string NextFundingDateString()
		{
			...
			var next_update_raw = lastUpdate + Instance._settings.fundingIntervalDays * dayLength;
		...
		protected void WindowGUI(int window_id)
		{
			...
			if (Instance._maintenance.isBuildingCostsEnabled) {
				GUILayout.BeginHorizontal(GUILayout.MaxWidth(_fundingWidth));
			...
			if (Instance._misc.assignedKerbalWage > 0) {
				GUILayout.BeginHorizontal(GUILayout.MaxWidth(_fundingWidth));
			...
			if (Instance._misc.baseKerbalWage > 0) {
				GUILayout.BeginHorizontal(GUILayout.MaxWidth(_fundingWidth));
			...
			if (Instance._misc.activeVesselCost > 0) {
				GUILayout.BeginHorizontal(GUILayout.MaxWidth(_fundingWidth));
			...
			if (Instance._misc.launchCostsLaunchPad + Instance._misc.launchCostsRunway > 0) {
				GUILayout.BeginHorizontal(GUILayout.MaxWidth(_fundingWidth));
...
//					showSettingsDialog = false;
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> When the vessel/plane rolls out of the VAB or SPH, the launch costs are calculated. These will
		...
		/// 		  runway requires minimal cost as compared to the launch-pad.</summary>
		///
		/// <param name="ship"> The ship that is being launched.</param>
		private void OnVesselRollout(ShipConstruct ship)
		{
			if (Instance == null) return;
			if (Instance._misc.launchCostsLaunchPad + Instance._misc.launchCostsRunway == 0) return;
			...
			if (ship.shipFacility == EditorFacility.VAB) {
				launch_cost = Instance._misc.launchCostsLaunchPad;
				facility_level = FacilityLevel(SpaceCenterFacility.LaunchPad);
			} else {
				launch_cost = Instance._misc.launchCostsRunway;
		...
		private int BaseStructureCost(SpaceCenterFacility facility)
		{
			...
			switch (facility) {
				case SpaceCenterFacility.Administration:
					return Instance._maintenance.structureCostAdministration;
				case SpaceCenterFacility.AstronautComplex:
					return Instance._maintenance.structureCostAstronautComplex;
				case SpaceCenterFacility.MissionControl:
					return Instance._maintenance.structureCostMissionControl;
				case SpaceCenterFacility.ResearchAndDevelopment:
					return Instance._maintenance.structureCostRnD;
				case SpaceCenterFacility.SpaceplaneHangar:
					return Instance._maintenance.structureCostSph;
				case SpaceCenterFacility.TrackingStation:
					return Instance._maintenance.structureCostTrackingStation;
				case SpaceCenterFacility.VehicleAssemblyBuilding:
					return Instance._maintenance.structureCostVab;
				case SpaceCenterFacility.LaunchPad:
				case SpaceCenterFacility.Runway:
					return 0;
				default:
					return Instance._maintenance.structureCostOtherFacility;
			}
		}
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> Should the funding period be logged in Kerbal Alarm Clock? If true, the player will be aware when
		/// 		  the funding period will end.</summary>
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> The home world which may be Kerbin, or not, according to what planet-pack has been installed.
		/// 		  Knowing the home world allows accurate calculation of days since the number of hours per day
		/// 		  might be different.</summary>
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> The last time the funding process was run. This is the time of the start of the fiscal period
		/// 		  that the funding was last processed.</summary>
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> Length of the year for the home system. This might be different than stock Kerbin as a result of
		/// 		  any planet-pack installed.</summary>
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> Builds an array of all the entries in the facilities enumeration so that iterating through the
		/// 		  facilities is possible.</summary>
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> True if the GUI should be visible. This might be turned false if the game signals it wants to
		/// 		  hide all GUI such as for a screen shot.</summary>
		private bool _visibleGui = true;


		///////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/// <summary> A record of the maintenance costs for the Space Center. This has to be stored rather than
